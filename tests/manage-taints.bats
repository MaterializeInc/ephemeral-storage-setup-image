...
setup() {
  ...
  export SECRET_DIR="${TEMP_DIR}/secrets/kubernetes.io/serviceaccount"
  mkdir -p "$SECRET_DIR"
...
@test "Shows usage when called with invalid action" {
  # Run the script with an invalid action
  run "$SCRIPT_PATH" invalid-action
  
  # Check output - should show usage
  echo "Output: $output"
  [ "$status" -eq 1 ]
  [[ "$output" == *"Usage: $SCRIPT_PATH [remove]"* ]]
}
